...
public:
  ...
  configuration() {
    opt_group{custom_options_, "global"}
      ...
      .add(pps, "pps,p", "set input probes per second")
...
void caf_main(actor_system& system, const configuration& config) {
  ...
  // A scope for the scoped actor.
  {
    ...
    // -- fake shards for counting ---------------------------------------------

    caf::actor icmp_dummy;
    caf::actor udp_dummy;

    auto accounting = system.spawn<caf::detached>(perf::counter);
    ...
    // -- start packet provider ------------------------------------------------

    // auto reader = system.spawn<caf::detached>(perf::packet_provider, shards,
    // config.pps);
