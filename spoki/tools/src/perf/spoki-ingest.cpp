...
public:
  ...
  configuration() {
    opt_group{custom_options_, "global"}
      ...
      .add(ingest_stats, "ingest-state,s", "print libtrace stats")
...
void caf_main(actor_system& system, const configuration& config) {
  ...
  // A scope for the scoped actor.
  {
    ...
    for (uint32_t i = 0; i < config.ingest_threads; ++i)
      shards.emplace_back(system.spawn<caf::detached>(perf::aggregator, accounting));
